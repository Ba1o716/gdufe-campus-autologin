...
*笔记本电脑在未接通电源情况下程序图标不会出现在任务栏中，但程序依旧能正常在后台运行，接通电源后会自动刷新出图标
...
### 6.6 用 Wi-Fi 也适用吗？——适用，但有几点要注意

程序判断的是"**能不能真正上网**"，不看你是插网线还是连 Wi-Fi；两种方式都能自动登录。
用 Wi-Fi（笔记本最常见）时注意这几件事：

1. **让电脑开机就自动连上校园 Wi-Fi**
   在 Windows「设置 → 网络和 Internet → WLAN → 管理已知网络」里，把校园 SSID 设为
   **自动连接**。否则开机后 Wi-Fi 没连上，程序会一直等到超时（默认 120 秒）并记一条
   "网络不可用"。
2. **关掉无线网卡的省电断网**
   「设备管理器 → 网络适配器 → 你的无线网卡 → 属性 → 电源管理」里，**取消勾选**
   "允许计算机关闭此设备以节约电源"；顺便把「高级」里的电源模式设为"最高性能"。
   否则待机 / 合盖后 Wi-Fi 掉线，认证会话也会跟着失效。
3. **同时插网线又连 Wi-Fi 时**
   程序会自动选"发往校园网门户那张网卡"的 IP 填进 `wlan_user_ip`，一般不用管。
   如果登录总是失败，先只保留一种连接方式试试（`--status` 里能看到它选了哪个 IP）。
4. **换教室 / 换 SSID / 换楼栋后 IP 变了**
   DHCP 会重新分配地址，原来的认证会话失效。程序的后台巡检（默认每 5 分钟）会发现
   "未认证"并自动重新登录，不需要你操作。想立刻恢复可以点托盘菜单的「立即登录」。
5. **需要验证码 / 短信验证的 Wi-Fi（例如访客网络）**
   这类网络程序无法自动完成，会提示你手动处理——这是设计如此，不会去绕过。

有线 ↔ 无线互相切换也一样：程序每次登录都会重新读当前的校园网 IP 和 AC 地址，
不需要改任何配置。
